...
public sealed class AllocateLicenseHandler : IRequestHandler<AllocateLicenseCommand>
{
    private readonly ILicenseRepository _repo;
    public AllocateLicenseHandler(ILicenseRepository repo) => _repo = repo;

    public async Task Handle(AllocateLicenseCommand request, CancellationToken cancellationToken)
    {
...
public sealed class ReleaseLicenseHandler : IRequestHandler<ReleaseLicenseCommand>
{
    private readonly ILicenseRepository _repo;
    public ReleaseLicenseHandler(ILicenseRepository repo) => _repo = repo;

    public async Task Handle(ReleaseLicenseCommand request, CancellationToken cancellationToken)
    {
        var pool = await _repo.FindByProductAsync(request.Product, cancellationToken);
        if (pool is null)
        {
